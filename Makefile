...
.PHONY: lint
lint: lint-ruff lint-codespell lint-mypy lint-pyright lint-shellcheck lint-yaml  ## Run all linters
	@# TODO: Re-enable lint-docs once we've fixed docs lint issues.
...
lint-mypy:  ##- Check types with mypy
	# Switch to this once we enable mypy on tests:
